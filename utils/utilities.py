...
class ReverseScaleToZeroToOne:
    def __call__(self, img):
        return (img + 1.0) / 2.0
    
# class ReverseScaleToZeroTo255:
#     def __call__(self, img):
#         # Reverse the ScaleToMinusOneToOne transformation
#         img = img.add(1.0).div(2.0)  # Scale back to [0, 1]
#         img = img.squeeze(0).permute(1, 2, 0).mul(255).clamp(0, 255).astype("uint8")  # Scale to [0, 255]
#         return img    

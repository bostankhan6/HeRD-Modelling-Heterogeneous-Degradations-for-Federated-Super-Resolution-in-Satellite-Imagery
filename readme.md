...
This repository contains the official source code and implementation for the paper "HeRD: Modelling Heterogeneous Degradations for Federated Super-Resolution in Satellite Imagery".
...
1.  **Configure Training Script**:
    ...
    * Update the following paths and parameters:
        ...
        * `HR_path_test` and `LR_path_test`: Point these to the `HR` and `LR_both` (or other variant) folders inside the `federated_validation` directory generated by the previous step.
...
2.  **Train the Federated Model**:
    * In `federated_training.py`, set:
        ...
        * `LR_path_test` to the `federated_validation/LR_both` folder inside the same generated directory.
        * `NUM_CLIENTS = 5`
